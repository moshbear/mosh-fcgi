/*! @file src/u.hpp - u_* stuff for src/\*
 *  
 *  This file is indentical to mosh/fcgi/bits/u.hpp, but it doesn't
 *  require the MOSH_FCGI namespace.
 *  Instead, it uses m.
 */
/***************************************************************************
 ...
 * @note
 * @note Unless @c SIGN_CAST_NO_CHAR_RESTRICT is defined, this function will only accept
...
std::enable_if<(
		...
		(std::is_pointer<To>::value && std::is_pointer<From>::value
			...
			&& std::is_same<typename std::make_unsigned<typename std::remove_pointer<To>::type>::type, 
				...
				>::value
#ifndef SIGN_CAST_NO_CHAR_RESTRICT
